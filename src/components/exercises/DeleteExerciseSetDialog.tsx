...
import type { ExerciseSet } from '@/validations/ResponseValidation';
import { AlertCircle } from '@untitledui/icons';
import { useTranslations } from 'next-intl';
import { buttonStyles, panelStyles } from '@/components/ui/styles';

type DeleteExerciseSetDialogProps = {
  errorMessage: string | null;
  isDeleting: boolean;
  onCancel: () => void;
  onConfirm: () => Promise<void>;
  setToDelete: ExerciseSet | null;
};

export function DeleteExerciseSetDialog(props: DeleteExerciseSetDialogProps) {
  const t = useTranslations('DashboardExercisesPage');

  if (!props.setToDelete) {
    return null;
  }
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-ink-950/45 px-4 backdrop-blur-sm">
      <div
        aria-labelledby="delete-exercise-set-title"
        aria-modal="true"
      ...
      >
        <div className="flex items-start gap-4">
          ...
          <h2 id="delete-exercise-set-title" className="text-lg font-semibold text-ink-950">
            {t('delete_set_title')}
          </h2>
        </div>

        <p className="mt-4 text-sm leading-6 text-ink-600">
          {t('delete_set_description', {
            count: props.setToDelete.exercises.length,
          })}
        </p>
        ...
        <div className="mt-5 flex justify-end gap-3">
          ...
          <button
            ...
            type="button"
          >
            {props.isDeleting ? t('delete_set_loading') : t('delete_set_confirm')}
          </button>
        </div>
      </div>
    </div>
